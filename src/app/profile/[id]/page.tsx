...
// Deduplicated within a single request: generateMetadata and the page
// component both need this player, so this makes sure they share one
// Supabase round-trip instead of firing two in sequence.
...
export async function generateMetadata({
  ...
  return { title: `${player?.full_name ?? "Carte"} — NEXUS CONTROL` };
}
...
export default async function ProfilePage({
  ...
  const verifyUrl = `${appUrl}/verify/${player.qr_token}`;
        ...
        <div className="hud-corner overflow-hidden rounded-xl border border-border bg-surface">
          <div className="flex items-center gap-4 border-b border-border px-5 py-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted">{player.activity}</p>
          ...
          <div className="flex flex-col items-center px-5 py-6">
            ...
            <p className="mt-4 text-center text-xs text-muted">
              Présentez ce QR code à un agent pour vérification. Il pointe
              vers votre fiche de contrôle publique.
            </p>
          </div>
        </div>
